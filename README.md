...
`μ_SFS`: The previous version of `μ_SFS` is multiplied by the (sample size) correcting factor of Watterson's θ: ` Sum_i=1^(n-1) 1/i`. Its values are now distributed around 1.0 under neutrality. 
...
The quick-reference help message generated by the current RAiSD release is the following.
      ...
      This is RAiSD version 2.2, released in January 2020.
      ...
      RAiSD
		...
		-R	Includes additional information (window start and end, and the mu-statistic factors for variation, SFS, and LD) in the report file(s).
		...
		-d	Provides a maximum distance from the selection target (in base pairs) to calculate the success rate, i.e., reported locations in the proximity of the target of selection (provided via -T).
