...
For each set of SNPs (separated by a line that contains the separator symbol "//" followed by the set index), the RAiSD_Report file contains a set of result lines, one per evaluated genomic location, with result line consisting of the genomic location followed by the calculated statistic values for the factors VAR, SFS, and LD, and finally the μ statistic. 
