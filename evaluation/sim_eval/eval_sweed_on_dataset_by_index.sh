...
v2=$(grep Position ../sim_runs/d$NUMBER/$TOOL\_Info.d$NUMBER\_selection\_$tool | awk -v N=2 '{print $2s}' | perl -e 'print sort{$a<=>$b}<>')
#IFS=$'\n' sorted=($(sort -n <<<"${v2[*]}"))
IFS=$'\n' sorted=($(cat <<<"${v2[*]}"))
...
v2=$(grep Likelihood ../sim_runs/d$NUMBER/$TOOL\_Info.d$NUMBER\_neutral\_$tool | awk -v N=2 '{print $2s}' | perl -e 'print sort{$a<=>$b}<>')
#IFS=$'\n' sorted=($(sort -n <<<"${v2[*]}"))
IFS=$'\n' sorted=($(cat <<<"${v2[*]}"))
...
v2=$(grep Likelihood ../sim_runs/d$NUMBER/$TOOL\_Info.d$NUMBER\_selection\_$tool | awk -v N=2 '{print $2s}' | perl -e 'print sort{$a<=>$b}<>')
#IFS=$'\n' sorted=($(sort -n <<<"${v2[*]}"))
IFS=$'\n' sorted=($(cat <<<"${v2[*]}"))
